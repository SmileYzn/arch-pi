...
# Ambiente
DE=$(xfce4-session --version | grep "xfce4-session 4.20")
...
if [[ $DE != "xfce4-session"* ]]; then
  echo -e "${VERMELHO}${NEGRITO}Ambiente ${VERDE}XFCE 4 Desktop${VERMELHO} não instalado.${REGULAR}${NORMAL}"
exit
fi
...
executar()
{
  ...
  case $1 in
    0)
      sudo pacman -S --needed --noconfirm 7zip base-devel bash-completion blueman fastfetch ffmpegthumbnailer git man power-profiles-daemon reflector system-config-printer unzip unrar xz zip
    ;;
    1)
    ...
    ;;
    3)
      sudo pacman -S --needed --noconfirm xorg-apps xorg-xwayland numlockx wayland labwc
    ;;
    4)
    ...
    ;;
    13)
      sudo pacman -S --needed --noconfirm catfish dconf-editor gnome-online-accounts gparted lightdm-gtk-greeter-settings mate-calc mpv mugshot seahorse zeitgeist
    ;;
    14)
...
while true; do
  ...
  for opcao in "${opcoes[@]}"; do
    ...
    if [ "$opcao" -ge 0 ] && [ "$opcao" -le 18 ]; then
      clear
      echo -e $DIVISOR
      executar $opcao
      echo -e $DIVISOR
